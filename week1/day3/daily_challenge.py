...
# letter_dict = {}
# index_user_word = 0 #the first index is 0, as new characters added, +1

# characters = list(user_word) #made each letter from user_word one list so can be used later

# while index_user_word < len(characters): #as long as the index is less than length of the user_word, continue 
#     letter = characters[index_user_word]

#     if letter not in letter_dict:
#         letter_dict[letter] = []

#     letter_dict[letter].append(index_user_word)

#     index_user_word += 1

# print(letter_dict)

    #need to start looping the letters and check if they are in the dictionary:
    # if they are, add the index; 
    # if they are not, add both the character and the index

    # for characters in user_word[1, len(user_word) + 1]:
    #     if characters not in word_dict.keys():
    #         word_dict.update({"character":index_user_word})
    #     else:
    #         #add the index of current count to the corresponding character, append to the end of the list there. 
...
if not basket:
    ...
    basket.sort()  # alphabetical order
    print(basket)
