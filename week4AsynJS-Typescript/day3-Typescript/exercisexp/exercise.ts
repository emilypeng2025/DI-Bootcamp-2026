...
//to check output: terminal on the same folder: tsc exercise.ts && node exercise.js, output correct: Hello, World!
...
// function judgement(a: number): string {
//     if (a > 0) {
//         return "positive";
//     } else if (a < 0) {
//         return "negative";
...
let judgement: (a: number) => string;
judgement = function (a: number): string {
    if (a > 0) {
        return "positive";
    } else if (a < 0) {
        return "negative";
...
function createPerson(name: string, age: number): Person {
    return {
        name: name,   //here the left name is property of object.name, the key of Person; the right name is parameter.name
        age: age
    };
}
...
// or: const input = <HTMLInputElement>document.getElementById("myInput"); //但在 React/TSX 不推荐用这个
...
//Function Overloading In TypeScript: You can define multiple “versions” of a function signature 
function greet(name: string): string;
function greet(): string;
//You can write multiple function signatures, BUT only ONE implementation
...
// Simpler version (no overloading)
//function greet(name?: string): string {
//     if (name) {
//         return `Hello, ${name}!`;
//     }
